...
``` r
remotes::install_github("BIMSBBioinfo/RSimpleITK")
```

Turn on mutlicore compilation, six cores in this example

``` r
remotes::install_github("BIMSBBioinfo/RSimpleITK", configure.vars=c("MAKEJ=6"))
```
...
``` r
remotes::install_github("BIMSBBioinfo/RSimpleITK", configure.vars=c("MAKEJ=6", "ADDITIONAL_SITK_MODULES=-DSimpleITK_USE_ELASTIX=ON\\  -DModule_ITKIODCMTK:BOOL=ON"))
```
